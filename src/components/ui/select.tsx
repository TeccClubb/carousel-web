...
>(({ label, size = "md", className, children, defaultIcon, ...props }, ref) => {
  const sizes = {
    ...
    <>
      ...
      <SelectPrimitive.Trigger
        ...
        className={cn(
          ...
          // "flex w-full items-center justify-between rounded-md border border-input bg-background gap-1 ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
          sizes[size],
          className
        )}
        {...props}
...
>(({ tickSide = "right", className, children, ...props }, ref) => (
  ...
  >
    <span
      className={`absolute ${tickSide}-3 flex h-3.5 w-3.5 items-center justify-center`}
    >
